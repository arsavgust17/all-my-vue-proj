<template lang="pug">
.showcase-body
    .table-header(v-if="columns.length")
        ...
        .column-titles
            .column-title-container.as-non-selectable(v-for="column in columns" :key="column.columnName" :class="column.classNames" :style="{width: column.columnWidth}")
                .column-title(v-if="column.columnVisible === 1" :class="{'as-pointer': column.isSortable}" v-html="column.columnName")
    ...
    .table-container
        Row( v-for="row in body" :key="row.DOC_KEY || Math.random()" :data="row" :columns="columns" :can-be-with-expander="!disabledExpander")
</template>
...
export default {
    // TODO выпилить из верстки всё лишнее, вероятно вернуть Loader для Row, см showcase/table
    name: 'showcaseTable',
    components: {
        Loader,
        BigSearch,
